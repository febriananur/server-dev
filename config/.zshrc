...
_auto_venv() {
  local GLOBAL_VENV="$HOME/.env/bin/activate"

  # --- deactivate ---
  if [[ -n "$VIRTUAL_ENV" ]]; then
    [[ -f main.py || -d .venv || -d venv || -f pyproject.toml || -f Pipfile ]] || deactivate 2>/dev/null
  fi
  ...
  if [[ -z "$VIRTUAL_ENV" ]]; then
    ...
    # global venv fallback (main.py based)
    [[ -f main.py && -f "$GLOBAL_VENV" ]] && source "$GLOBAL_VENV"
  fi
}
